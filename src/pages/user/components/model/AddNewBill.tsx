...
import { m_payment, member } from "../../../../assets/types/user";
...
const AddNewBill = ({
  ...
  const m_pay = useMutation({
    mutationFn: async (data: m_payment) => {
      return await userTokenAxios.post(
        `/add-member-payment/${m_id}`,
        JSON.stringify(data)
      );
    },
    onSuccess: (response) => {
  ...
  const u_pay = useMutation({
    ...
    onSuccess: (response) => {
      ...
      console.log(
        response,
        queryClient.getQueryData<AxiosResponse>(["all-member-data"])
      );
    },
  });
  const d_pay = useMutation({
    mutationFn: async (mp_id: number | undefined) => {
      return await userTokenAxios.delete(`/delete-member-payment/${mp_id}`);
    },
    onSuccess: (response) => {
      console.log(response.data.member_data);
      reset();
  ...
  const onSubmit: SubmitHandler<m_payment> = async (para_data: m_payment) => {
      ...
      >
        <Fade in={open}>
          <Box sx={{ ...modelStyle, maxHeight: "90%" }}>
            <Header
              text={update ? "Edit Bill" : "New Bill"}
              css={{
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                ...
                <Input
                  ...
                  defaultVal={
                    update
                      ? mData?.M_Payments && mData?.M_Payments[0]?.start_date
                      : currentDate
                  }
                />
                <Input
                  ...
                  defaultVal={
                    update
                      ? mData?.M_Payments && mData?.M_Payments[0]?.end_date
                      : ""
                  }
                />
                ...
                <Input
                  ...
                  defaultVal={
                    update
                      ? mData?.M_Payments && mData?.M_Payments[0]?.fee
                      : "800"
                  }
                />
                <Input
                  ...
                  defaultVal={
                    update
                      ? mData?.M_Payments && mData?.M_Payments[0]?.total_month
                      : "1"
                  }
                />
              </Stack>
              <Stack flexDirection={"row"} justifyContent={"space-between"}>
                ...
                {update ? (
                  <CustomButton
                    ...
                    type={"button"}
                    func={() =>
                      d_pay.mutate(mData?.M_Payments && mData?.M_Payments[0].id)
                    }
                  />
                ) : (
                  <CustomButton
